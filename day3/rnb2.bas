...
lifeLostScr = _NewImage(500, 500, 32)
score = 0
lives = 0
Cls
...
_Dest lifeLostScr
_PrintString (170, 250), "Get Ready"

_Dest titleScr
Color _RGB(255, 255, 0)
Circle (200, 200), 90, _RGB(0, 0, 255)
Circle (260, 260), 90, _RGB(255, 0, 0)
...
' BLUE BLOCKS====================
_Dest blockSprBlue
Line (1, 1)-(39, 99), _RGB(0, 0, 255), BF
...
_Dest gameOverScr
_PrintString (170, 250), "game over dude!!"
...
lifeLostScrH = _CopyImage(lifeLostScr, 33)
_FreeImage lifeLostScr
...
_PutImage , lifeLostScrH
If Int(retryTTL / 8) Mod 2 = 0 Then
    Select Case p.state
        Case 0:
...
For i = 0 To 99
    Select Case enemies(i).state
        ...
        Case 1:
            _PutImage (enemies(i).x, enemies(i).y), blockSprBlueH
    End Select
...
For i = 0 To 99
    ...
    If enemies(i).state > -1 Then
        If (actorCollideSpecial(p, enemies(i)) = 1) Then
            If p.state = enemies(i).state Then
                score = score + 1
                If p.state = 0 Then
                    p.state = 1
                Else
...
If ttl = 0 Then
    If (enemies(nextBlankEnem).state = -1) Then
        enemies(nextBlankEnem).x = Int(Rnd * 20) * 15
...
_Dest gameOverScr
_PrintString (170, 250), "game over dude!!"
_PrintString (170, 280), "final score was " + Str$(score)
_Dest 0
gameOverScrH = _CopyImage(gameOverScr, 33)
_FreeImage gameOverScr
